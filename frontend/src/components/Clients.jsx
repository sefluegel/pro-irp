          ...
          {columnDefs.map(col=> (
            <label key={col.key} className="flex items-center gap-2 text-sm mb-1">
              <input
                ...
                onChange={e=> setColumns(cs=> e.target.checked ? [...cs,col.key] : cs.filter(c=>c!==col.key))}
              /> {col.label || col.key}
            </label>
          ))}
        </div>
          ...
          <thead className="bg-[#172A3A] text-white">
            <tr>
              {columnDefs.filter(c=>columns.includes(c.key)).map(col=> (
                <th key={col.key} className={`py-2 px-2 ${col.width||''}`}>{col.key==='select' ? <input type="checkbox" checked={selected.length===filteredClients.length} onChange={handleSelectAll}/> : col.label}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {filteredClients.length===0 ? (
              <tr><td colSpan={columns.length} className="py-8 text-center text-gray-400">No clients found.</td></tr>
      ...
      {showAdd && (
        <div className="fixed inset-0 z-40 bg-black/30 flex items-center justify-center">
          <div className="bg-white rounded-xl shadow-2xl p-6 w-[380px] max-w-full relative">
            <button className="absolute top-2 right-2 text-gray-400 hover:text-black" onClick={()=>setShowAdd(false)}><X size={18}/></button>
            ...
            <form className="space-y-2">
              ...
              <div className="flex gap-2">
                ...
                <select className="flex-1 px-3 py-2 border rounded text-sm"><option>Status</option><option>Active</option><option>Prospect</option><option>Inactive</option></select>
              </div>
              <input className="w-full px-3 py-2 border rounded text-sm" placeholder="Owner/Agent" />
              <button type="button" onClick={()=>setShowAdd(false)} className="w-full bg-[#FFB800] hover:bg-yellow-400 text-[#172A3A] font-bold py-2 rounded-xl shadow">Save (Demo)</button>
            </form>
          </div>
        </div>
      )}
