...
const DEMO_CLIENTS = [
  ...
  },
  {
    ...
    name: "Zain Marketing",
    initials: "ZM",
    phone: "+1 905 693 6475",
    email: "zainmarketingz8@gmail.com",
    created: "Jul 09 2025",
    lastActivity: "4 days ago",
    tags: ["PDP"],
    risk: "Low",
  ...
  {
    id: 4,
    name: "Janik Lilienthal",
    initials: "JL",
    phone: "(859) 333-5555",
    email: "jlilienthal@gmail.com",
    created: "Jul 09 2025",
    lastActivity: "4 weeks ago",
    tags: [],
    risk: "Medium",
    ...
    <div className="p-6 max-w-[1300px] mx-auto">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex gap-2">
          <button
            ...
            className="flex items-center gap-1 px-3 py-1 rounded bg-white border shadow-sm hover:bg-gray-50 transition text-xs"
          >
          ...
          {columnDefs.map((col) => (
            <label key={col.key} className="flex items-center gap-2 text-sm mb-1">
              <input
                ...
                onChange={(e) =>
                  setColumns((cs) =>
                    e.target.checked ? [...cs, col.key] : cs.filter((c) => c !== col.key)
                  )
                }
              />
              {col.label || col.key}
          ...
          <thead className="bg-[#172A3A] text-white">
            <tr>
              {columnDefs.filter((c) => columns.includes(c.key)).map((col) => (
                <th key={col.key} className={`py-2 px-2 ${col.width || ""}`}> 
                  {col.key === "select" ? (
                    <input
                      type="checkbox"
            ...
            {filteredClients.map((c) => (
              <tr key={c.id} className={`${c.unread ? "bg-blue-50" : ""} border-b hover:bg-yellow-50`}>                
                ...
                {columns.includes("initials") && (
                  <td className="py-1 px-2">
                    <div className="w-7 h-7 rounded-full bg-[#172A3A] text-white flex items-center justify-center font-bold text-xs">
                      {c.initials}
                    </div>
                  </td>
                )}
                ...
                {columns.includes("risk") && (
                  <td className="py-1 px-2">
                    <span className={`px-2 py-0.5 rounded-full text-xs font-bold ${RISK_COLORS[c.risk]}`}>{c.risk}</span>
                  </td>
                )}
                {columns.includes("status") && (
                  <td className="py-1 px-2">
                    <span className={`px-2 py-0.5 rounded-full text-xs font-bold ${STATUS_COLORS[c.status]}`}>{c.status}</span>
                  </td>
                )}
      ...
      {showAdd && (
        <div className="fixed inset-0 z-40 bg-black/30 flex items-center justify-center">
          <div className="bg-white rounded-xl shadow-2xl p-6 w-[380px] max-w-full relative">
            <button onClick={() => setShowAdd(false)} className="absolute top-2 right-2 text-gray-400 hover:text-black"><X size={18} /></button>
            <h2 className="text-xl font-bold text-[#172A3A] mb-2">Add Client</h2>
            <p className="text-gray-500 text-xs mb-3">(*Demo only: fields not connected)</p>
            <form className="space-y-2">
              ...
              <div className="flex gap-2">
                <select className="flex-1 px-3 py-2 border rounded text-sm"><option>Risk</option><option>High</option><option>Medium</option><option>Low</option></select>
                <select className="flex-1 px-3 py-2 border rounded text-sm"><option>Status</option><option>Active</option><option>Prospect
