...
const Login = () => {
  ...
  const handleSubmit = async e => { e.preventDefault(); setMsg("Logging in..."); try { const { token } = await api.login({ email, password: pw }); localStorage.setItem("token", token); setMsg(""); navigate("/dashboard"); } catch (err) { setMsg("Login failed: " + err.message); } };
