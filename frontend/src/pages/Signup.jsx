...
const Signup = () => {
  ...
  const handleSubmit = async e => { e.preventDefault(); setMsg("Creating account..."); try { const { token } = await api.signup({ email, password: pw, name: email.split("@")[0] }); localStorage.setItem("token", token); setMsg(""); navigate("/dashboard"); } catch (err) { setMsg("Signup failed: " + err.message); } };
